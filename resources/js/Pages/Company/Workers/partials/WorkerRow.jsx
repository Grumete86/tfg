...
export default function WorkerRow({ worker, updateFunction }) {
    ...
    return (
        <>
            <a href={route('company.workers.show', { worker: actualWorker })}>
                <div class="bg-white transition-all shadow-lg rounded-lg w-80 dark:bg-gray-800 hover:bg-slate-400 group group-hover:text-white hover:cursor-pointer">
                    {/* <img alt="profil" src="https://random.imagecdn.app/320/150" class="w-full mb-4 rounded-t-lg h-28" /> */}
                    <div class="flex flex-col items-center justify-center p-4 mt-2">
                        <img
                            alt="profil"
            ...
            {/* <div className="col-span-2 flex flex-col gap-3">
                <Status handleStartEndShift={handleStartEndShift} workerStatus={actualStatus}
                />
                <div className="flex flex-row gap-3 flex-wrap flex-1 justify-start overflow-hidden">
                    <a href={route('company.workers.show', { worker: actualWorker })} className="hover:text-indigo-500">
                        <div>
                            <p className="text-slate-500" style={{ fontSize: '0.5rem' }}>NOMBRE</p>
                        </div>
                        {actualWorker.name}

                        <div>
                            <p className="text-slate-500" style={{ fontSize: '0.5rem' }}>DNI</p>
                        </div>
                        {actualWorker.dni}
                    </a>
                    <a href={`mailto:${actualWorker.email}`} className="hover:text-indigo-500">
                        <div>
                            <p className="text-slate-500" style={{ fontSize: '0.5rem' }}>EMAIL</p>
                        </div>
                        {actualWorker.email}
                    </a>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 justify-evenly">
                <Interval title='Horas Totales' interval={totalWorked} className="" />
                <Interval title='Horas Mes' interval={totalWorkedMonth} className="" />
            </div> */}
        </>
    );
}
